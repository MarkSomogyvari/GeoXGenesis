...
def read_epos_sta_coords_mono(filein):
    """
    ...
    for l in F:
        ...
        if "SITE" in fields[0]:
            namestat = fields[8]
            MJD_strt = float(fields[5])
            MJD_end  = float(fields[7])
...
def read_bull_B(path):
    ...
    S = genefun.extract_text_between_elements(path,"1 - DAILY FINAL VALUES" ,
    ...
    DF.columns = ["year","month","day","MJD","x","y","UT1-UTC","dX","dY",
...
def read_sp3(file_path_in,returns_pandas = True, name = '',
    ...
    for l in fil:
        ...
        if l[0] == '*':
            ...
            if returns_pandas:
                line_data = [epoc,sat_sat,sat_nat,sat_sv,X,Y,Z,Clk]
    ...
    if returns_pandas:
        ...
        return  epoch_stk ,  Xstk , Ystk , Zstk , Clkstk
...
def export_ts(ts,outdir,coordtype = 'ENU',outprefix=''):
    ...
    for a,b,c,t,sa,sb,sc in zip(A,B,C,T,sA,sB,sC):
        ...
        outlin =  '{:18} {:18} {:18} {:18} {:18} {:18} {:18} {} {:20} {} {}'.format( *(paramstr + [tt.strftime("%Y %m %d %H %M %S"), str(geok.dt2year_decimal(tt)),str(t) ,'\n']))
        filobj.write(outlin)
    filobj.close()
