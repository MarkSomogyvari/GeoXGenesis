# -*- coding: utf-8 -*-
"""
Created on Sat Mar 14 15:58:47 2015
...
def copy_file_in_gin_folder(a_file_path,repository_folder='/home/psakicki/gin/TP/GWADA/RINEX'):
...
def write_oceanload_file(station_file,oceanload_out_file):
    ...
    p = subprocess.Popen('exe_loadoce_fes2012 < ' + temp_cmd_fil, shell=True , 
                         executable='/bin/bash',
...
def prairie_manual(rinex_paths_in , temp_data_folder = '' , 
                   force = True , with_historik = True, with_wsb = True ,
                   argsdict = dict()):
...
#        # ... and copy it otherwise
#        if not bool_rinex_in_gin:
        expected_rinex_path = os.path.join(temp_data_folder,os.path.basename(rinex_path))
        ...
        if os.path.isfile(out_pra_path) and not force:
            print("INFO : prairie_manual : ", out_pra_path , 'already exists, skiping ...')
        ...
        if not '-cc2noncc' in list(argsdict.keys()):
            argsdict['-cc2noncc'] = os.path.join(get_gins_path(1),'archives/p1c1bias.2000p')
        if with_historik and not '-historik' in list(argsdict.keys()): 
            argsdict['-historik'] = os.path.join(get_gins_path(1),'data/constell/historik_glonass')
        ...
        if not '-options' in list(argsdict.keys()):
            argsdict['-options']  = os.path.join(get_gins_path(1),'data/prairie/options_GPS.dat') 
...
def gen_dirs_from_rnxs(rinex_paths_in,director_generik_path,
                       director_name_prefix,out_director_folder='',
                       temp_data_folder='',stations_file='',
                       oceanload_file='',auto_staocl=False,
                       perso_orbclk=False,calc_center='igs',repro=2,
    ...
    for i,rinex_path in enumerate(rinex_path_lis):
        ...
        if prairie:
            ...
            pra_file_path = prairie_manual(rinex_path,temp_data_folder,**prairie_kwargs)
            
            if type(pra_file_path) is list and len(pra_file_path) == 0:
                print("ERR : something went wrong during prairie externally, skip the day")
                print(rinex_path)
        ...
        director_output_name = director_name_prefix + '_' + stat_lower + '_' + \
        str(geok.dt2jjulCNES(dt_rinex)) + '_' + str(dt_rinex.year) + '_' + \
        geok.dt2doy(dt_rinex) + freq_rnx_str + coord_prefix + ses + \
        calccntr_suffix + '.yml'
        director_output_path = os.path.join(out_director_folder,director_output_name)
        director_generik_file = open(director_generik_path)
        director_dic = yaml.load(director_generik_file)    
        ...
        if not prairie:
            ...
            director_dic['observation']['interobject_data'][1]['file'] = pra_path_dir_compatible
        ...
        if auto_interval: # dir is modified only if freq_rnx >= 0
            ...
            if director_dic['parameter']['adjustment_parameters']['stations']['adjustment_frequency'][-1] != 0:
                director_dic['parameter']['adjustment_parameters']['stations']['adjustment_frequency'] = [0, 0, 0, freq_rnx_true]
        print("INFO : auto_interval", auto_interval , ', simulation_stepsize : ' , director_dic['observation']['removal']['simulation_stepsize'])
        # output coords
        if out_coords.upper() in ('FLH','PLH'):
            director_dic['parameter']['adjustment_parameters']['stations']['coordinates'] = "latitude_longitude_height"
        elif  out_coords.upper() == 'XYZ':
            director_dic['parameter']['adjustment_parameters']['stations']['coordinates'] = "cartesian_xyz"
        ...
        #        gr_gr2 = 'GR1' # obsolete parce que GR1 intégralement inclu dans GR2
            ...
            if orbpath == None or horpath == None:
                print("WARN : something went wrong with orbits/clocks download/conversion " , dt_rinex)
        ...
        if 'user_extension' in director_dic:
            ...
            if 'userext_addition' in director_dic['user_extension']:
                if 'gps__haute_freq' in director_dic['user_extension']['userext_addition']:
                    director_dic['user_extension']['userext_addition']['gps__haute_freq']         = stat
                if 'gps__haute_freq'.upper() in director_dic['user_extension']['userext_addition']:
                    director_dic['user_extension']['userext_addition']['gps__haute_freq'.upper()] = stat 
        if 'userext_addition' in director_dic['user_extension']: 
            if np.any(['gps__haute_freq'.upper() in e for e in director_dic['user_extension']['userext_addition']]):
                print("INFO : kinematic process with 'userext_addition' fields")
                index_gps_hf = ['gps__haute_freq'.upper()  in e for e in director_dic['user_extension']['userext_addition']].index(True)
                director_dic['user_extension']['userext_addition'][index_gps_hf] = 'gps__haute_freq '.upper() + stat
...
def gen_dirs_from_double_diff(dd_files_paths_in,director_generik_path,
                       director_name_prefix,out_director_folder='',
                       temp_data_folder='',stations_file='',
                       oceanload_file='',auto_staocl=False,
    ...
    for i,dd_path in enumerate(dd_files_paths_in):
        ...
        # interval
        if auto_interval: # dir is modified only if freq_rnx >= 0
            director_dic['observation']['removal']['simulation_stepsize'] =  int(freq_dd_true)
            # check if its not a static dir
            if director_dic['parameter']['adjustment_parameters']['stations']['adjustment_frequency'][-1] != 0:
                director_dic['parameter']['adjustment_parameters']['stations']['adjustment_frequency'] = [0, 0, 0, freq_dd_true]
        print("INFO : auto_interval", auto_interval , ', simulation_stepsize : ' , director_dic['observation']['removal']['simulation_stepsize'])
        # output coords
        if out_coords.upper() in ('FLH','PLH'):
            director_dic['parameter']['adjustment_parameters']['stations']['coordinates'] = "latitude_longitude_height"
        ...
        if 'user_extension' in director_dic:
            ...
            if 'userext_gps__haute_freq'.upper() in director_dic['user_extension']:
                director_dic['user_extension']['userext_gps__haute_freq'.upper()] = stat_B 

            if 'userext_addition' in director_dic['user_extension']:
                if 'gps__haute_freq' in director_dic['user_extension']['userext_addition']:
                    director_dic['user_extension']['userext_addition']['gps__haute_freq']         = stat_B
                if 'gps__haute_freq'.upper() in director_dic['user_extension']['userext_addition']:
                    director_dic['user_extension']['userext_addition']['gps__haute_freq'.upper()] = stat_B 
                    
        if 'userext_addition' in director_dic['user_extension']: 
            if np.any(['gps__haute_freq'.upper() in e for e in director_dic['user_extension']['userext_addition']]):
                print("INFO : kinematic process with 'userext_addition' fields")
                index_gps_hf = ['gps__haute_freq'.upper()  in e for e in director_dic['user_extension']['userext_addition']].index(True)
                director_dic['user_extension']['userext_addition'][index_gps_hf] = 'gps__haute_freq '.upper() + stat_B
...
def run_directors(dir_paths_in,opts_gins_pc='',opts_gins_90='  ',
                  version='OPERA',fic_mode=False):
    """
    ...
    for i,director_path in enumerate(director_path_lis):
        ...
        if not fic_mode:
            command = "ginspc.bash " + opts_gins_pc_ope + ' ' + director_name + ' ' + opts_gins_90 + ' -v ' + version
        else:
...
def run_dirs_multislots(director_lis,slots_lis=['','U','L','R'] ,
                        opts_gins_pc='' , opts_gins_90='', version='OPERA',
                        fic_mode = False):
...
def get_director_list(wildcard_dir):
    ...
    gins_path = get_gins_path()
    di_run_lis = [ os.path.basename(e) for e in glob.glob(os.path.join(gins_path,'data','directeur',wildcard_dir)) ]
    return di_run_lis   

def get_rinex_list(parent_folder,specific_stats=[],invert=False,compressed=True,
                   start=dt.datetime(1980,1,1),end=dt.datetime(2099,1,1)):
    """ return :
    ...
    if specific_stats != []:
        if not invert:
            goodrnxfilelist = [fil for fil in rinexfilelist if os.path.basename(fil)[0:4] in specific_stats]
        else:
            goodrnxfilelist = [fil for fil in rinexfilelist if os.path.basename(fil)[0:4] not in specific_stats]        
    else:
    ...
    if goodrnxfilelist == []:
        print("WARN : get_rinex_list : no RINEX found, check the path of the parent folder !")
    else:
    ...
    if len(goodrnxfilelist_date) != len(goodrnxfilelist):
        ...
        print('INFO : get_rinex_list :' , dif , 'RINEXs removed bc. not in the date interval') 
...
def smart_directors_to_run(wildcard_dir='',full_path_out=True):
    ...
    # 2) treating NEW case
    # MAKING LISTS OF TUPLES (PREFIX , <gins or prepars>)
    li_tuple_lis_new = [(e.split('.')[0] + '.' + e.split('.')[1],e.split('.')[-1]) for e in lilis_new]
...
def smart_listing_archive(wildcard_dir,gins_main_archive,gins_anex_archive,
                          prepars_archive,director_archive):
    ...
    listing_path = os.path.join(get_gins_path(), 'gin','batch','listing',wildcard_dir)
...
#    gins_tuple_lis  = [(e.split('.')[0] , e.split('.')[2] ,e.split('.')[3]) for e in gins_lis]
    ...
    for m in multi_gins_prefix:
        ...
        multi_gins_fullpath_anex = multi_gins_fullpath_anex + multi_gins_fullpath_temp[1:]
    
    for directory in [gins_main_archive,gins_anex_archive,prepars_archive,director_archive]:
...
def clk_2_gins(clk_pathin):
    ...
    kommand = "clk2gins.ksh " + os.path.basename(clk_pathin)
#    stream = os.popen(kommand) # forrtl: severe if we use os.popen
#    subprocess.call([kommand], shell=True)
    p = subprocess.Popen([kommand], shell=True , stdout=subprocess.PIPE , stderr=subprocess.PIPE)
    out, err = p.communicate()
...
def download_convert_2_gins_orb_clk(centraldate, work_folder = '' ,
    ...
    if temp_dir == '':
        temp_dir = os.path.join(get_gins_path(),'TEMP_DATA')
    ...
    if sp3Zlis == [] or clkZlis == []:
        print("ERR : download_convert_2_gins_orb_clk : missing sp3/clk for" , centraldate)
...
def export_results_gins_listing(listings_list_in,outpath,
                                static_or_kinematic='kine',
                                outprefix='',coordtype='FLH'):
...
def double_diff_binom(rinex_path_A,rinex_path_B,temp_data_folder='',
                      final_data_folder = ''):
    """
    DISCONTINUED
    """
...
def double_diff_multi(rinex_path_list,temp_data_folder='',
                      final_data_folder = '',remove=True,
    ...
    for rinex_path in rinex_path_list:
        ...
        praA_path = prairie_manual(rinex_path,temp_data_folder=temp_data_folder,
                                               argsdict=argdict)
